 ...
 * show data
 */
function wedevs_get_address($args=[]){
...
/**
 * the count total address
 */

function weDevs_count_address(){
    global $wpdb;
    return (int) $wpdb->get_var("SELECT count(id) FROM {$wpdb->prefix}addresses");
}

/**
 * Edit single address in the list table
 *
 */
function wedevs_get_single_address($id)
{
    global $wpdb;
    $address=wp_cache_get('book-' . $id, 'address');
    if($address===false){
        $address = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}addresses WHERE  id=%d", $id));
...
/**
 * delete single address in the list
 * @param $id
 * @return bool|int
 */
function wedevs_delete_single_address($id){
    global $wpdb;
